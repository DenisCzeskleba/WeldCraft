...
IMPORTANT:
    - Ensure dx == dy unless solver logic has been updated for anisotropic meshing.
    - Double-check solver stability when modifying diffusion coefficients or dt.
    - Hydrogen diffusion coefficient is temperature-dependent:
      D_H = 8.8e-9 * T ** 2.2285  [mm²/s]
"""
...
t_conv_air = 4e-5  # [W/mm²/K], ≈ 40 W/m²K, top plate, still air. Guess / calibrate from temperature measurements
...
# diff_coeff_h = 6.8016 * 10 ** - 8  # Low value from our project (QL BM)
diff_coeff_h = 7.7908 * 10 ** - 8  # high value from our project (TM BM)
...
# ----------------------------------------------- Time Differential  --------------------------------------------------
safety_factor = 1  # should usually be ok to just use 1
...
use_big_dt_override = False  # use the 600s save time override for the last stage (diffusion at RT)
...
no_of_weld_beads = 22  # no of "blocks" during welding: Butt joint: must be %2, fit bead_height! Lap Joint: max 4
bead_height = 3  # Half of weld beads * height should probably be weld thickness (th) (2.8 for iso?
bead_width = 15  # Using half of weld width (we) for blocks, for ellipses maybe 3/4-ish of weld width?
...
time_before_first_weld = 24  # Run simulation before starting with first bead. Not necessary, makes for nicer videos [s]
time_for_weld_bead = 300  # Time between welds. Weld block gets added at 0. Temp. held for time_heat_hold. [s]
time_after_last_weld = 1  # Time after last weld. BC in sample edge is held at t_cool this long. [s]
...
time_diffusion_at_rt = 3 * 24 * 60 * 60  # 2d * 24h * 60min * 60s
...
file_name = "diffusion_array.h5"  # diffusion_array.h5"
animation_name = "diffusion_animation.mp4"  # diffusion_animation.mp4
s_per_frame_part1 = 5.0
# ---------------------------------------------------------------------------------------------------------------------

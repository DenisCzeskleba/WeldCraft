...
# this was just a save while you worked on the other joint for Kjell.
...
dx = 0.50  # step size in x direction - if not equal to dy, tripple check solver logic!
dy = 0.50  # step size in y direction - if not equal to dx, tripple check solver logic!
...
diff_coeff_h_static = 1.5 * 10**-5  # Not used! No static h-diffusion!! **2
...
no_of_weld_beads = 22  # no of "blocks" to add during welding - make sure its %2 and fits bead_height!
bead_height = 3  # Half of weld beads * height should probably be weld thickness (th)
...
hydro_weld_metal = 6  # Hydrogen in the new weld block

time_before_first_weld = 6  # Run simulation before starting with first bead. Not necessary, makes for nicer videos [s]
time_for_weld_bead = 600  # Time between welds. Weld block gets added at 0. Temp. held for time_heat_hold. [s]
time_after_last_weld = 300  # Time after last weld. BC in sample edge is held at t_cool this long. [s]
...
time_cooling_to_rt = 5400  # For now set to 1.5h. During, forced linear cooling as BC in sample metal

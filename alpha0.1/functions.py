...
def get_value(param_name):
    ...
    if hasattr(param_config, param_name):
        return getattr(param_config, param_name)
    else:
...
def manipulate_simulation(sim_type, u0, h0, D, D_H, cwi, cci, t_weld_metal, h_weld_metal, D_bm, D_weld_metal, D_haz,
                          D_H_weld_metal, dx, dy, le, we, fr_ab, fr_le, th, mask, faces):

    if sim_type == "butt joint":

        # Get the bead height and width from param_config - will be used for the ellipse later
        ...
        bead_x2 = int((fr_ab + th - (get_value("bead_height") * (cwi // 2))) / dx)  # odd solution but we need do avoid doing /dx twice
        bead_y1 = int(le / dy) + 1
        bead_y3 = int((le + we) / dy)

        # figure out the center of the ellipse we are gonna use later
        if cwi % 2 == 0:
            center_x, center_y = bead_x2, bead_y1
        else:
            center_x, center_y = bead_x2, bead_y3

        # Get the shape of the matrix, gives the number of rows and columns
        num_rows, num_cols = u0.shape

        # Create coordinate grids for rows and cols directly from u0's .shape
        ...
        if cwi % 2 == 0:  # Even - left bead
            ...
            # Check if the points are inside the ellipse, dont allow 3rd quadrant
            # If messing with this, double check < | <= etc.!!
            inside_ellipse = (distance <= 1) & ~((row_grid > center_x) & (col_grid < center_y)) & (D != D_bm) & (
                    D != D_haz)

        else:  # Odd - right bead

            # Check if the points are inside the ellipse, dont allow 4rd quadrant
            # If messing with this, double check < | <= etc.!!
            inside_ellipse = (distance <= 1) & ~((row_grid > center_x) & (col_grid > center_y)) & (D != D_bm) & (
                    D != D_haz)

        # Assign values to points inside the ellipse
        u0[inside_ellipse] = t_weld_metal
        h0[inside_ellipse] = h_weld_metal

        # If this is the first time this is triggered, also adjust D
        if cci == 0 and not isinstance(D, (int, float)):  # adjust D
            D[inside_ellipse] = D_weld_metal
            D_H[inside_ellipse] = D_H_weld_metal
            mask, faces = compute_mask_and_faces(D)  # update our simulation area

# Two-dimensional diffusion partial differential equation with a location-dependent diffusion coefficient
import sys
...
if add_bead_mode == "regular_intervals":
    ...
    print("A new weld bead will be added at the following times (s):")
    print(change_times)
...
while current_time <= total_max_time:
    ...
    # ---- WELDING PHASE ----
        ...
        if change_times[cwi] <= current_time <= change_times[cwi] + keep_temp_duration:
            ...
            # Change stuff, like adding a weld bead and such (in functions.py)
            # Info: cci is used to determine if we should also manipulate the diffusion values
            u0, h0, D, D_H, mask, faces = manipulate_simulation(simulation_type, u0, h0, D, D_H, cwi, cci, t_weld_metal,
                                                                h_weld_metal, D_bm, D_weld_metal, D_haz, D_H_weld_metal,
                                                                dx, dy, le, we, fr_ab, fr_le, th, mask, faces)

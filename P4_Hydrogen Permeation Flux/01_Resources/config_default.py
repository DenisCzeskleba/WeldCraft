...
DEFAULT_CONFIG = {
    ...
    "trapping": {
        ...
        "capture_sweep_release_half_time_ref": 1.0,
        "capacity_ratios": [0.0, 0.5, 1.0, 1.5, 2.0, 3.0, 5.0, 10.0],
        "capacity_sweep_release_half_time_ref": 1.0,
        "capacity_sweep_binding_energy_kJ_mol": 30.0,
        "release_half_times_ref": [0.1, 0.25, 0.5, 1.0, 2.0],
        "release_sweep_capacity_ratio": 1.0,
        # User-facing strength sweep. These energies are converted to a
        # detrapping rate with the calibration printed on the trapping figure.
        "binding_energy_kJ_mol": list(map(float, range(20, 101))),
        "binding_energy_temperature_K": 293.15,
        "detrapping_prefactor_s_inv": 5.0e3,
        "lattice_activation_energy_kJ_mol": 4.5,
        "combined_cases": [
            {"label": "trap-free", "capacity_ratio": 0.0, "release_half_time_ref": 1.0},

...
# --- Hot-reload helpers (file fingerprint + camera snapshot) ---
import hashlib
from pathlib import Path as _Path

def _file_fingerprint(path):
    """Return (size, blake2b16) for change detection; None if missing."""
    try:
...
def _capture_camera(pl):
    cam = pl.camera
    return (cam.GetPosition(), cam.GetFocalPoint(), cam.GetViewUp())

def _restore_camera(pl, snap):
    if not snap:
        return
    pos, foc, up = snap
    pl.camera.SetPosition(*pos)
    pl.camera.SetFocalPoint(*foc)
    pl.camera.SetViewUp(*up)
# --- Green-arrow / default config support ---
CONFIG_OVERRIDE: Optional[str] = None
DEFAULT_CONFIG_BASENAME = "User_Input.yaml"
...
# ------------------ Render ------------------
def plot(cfg: Config, export_dir: Optional[str], export_merged: Optional[str], screenshot: Optional[str], no_show: bool, cfg_path: Optional[str] = None):
    """
Assemble scene, choose render path, handle overlay/picking, and show/export.
    ...
    # --- Hot reload: F5 + auto-watch YAML ---
    # Only active when showing UI and when a config file path is known
    if (not no_show) and cfg_path:
        _last_fp = _file_fingerprint(cfg_path)
        _reload_cooldown_ms = 400
        _state = {"last_reload_time": 0.0}

        def _rebuild_scene_in_place(new_cfg: Config):
            cam_snap = _capture_camera(pl)
            try:
                try:
                    clear_toast(pl)
                except Exception:
                ...
                try:
                    ...
                    pass
                pl.set_background(new_cfg.background)

                base_world, dop_meshes, dopant_world_centers = build_scene_points(new_cfg)
                n_atoms = 0 if base_world is None else base_world.shape[0]

                use_impostor = (new_cfg.render_mode == "impostor_points")
                use_instanced = (new_cfg.render_mode in ("auto", "spheres")) and HAVE_GLYPH3D_MAPPER and (n_atoms > 0)

                if use_impostor:
                    add_points_impostor(pl, base_world, new_cfg.base_color, new_cfg.points_impostor_size)
                elif use_instanced:
                    theta, phi = adaptive_base_res(n_atoms, new_cfg)
                    chunks = (chunk_points_z(base_world, new_cfg.chunk_target_atoms, new_cfg.chunk_max_actors)
                              if new_cfg.chunking_enabled and n_atoms > 0 else [base_world])
                    for ch in chunks:
                        actor = make_instanced_actor(ch, new_cfg.base_radius, new_cfg.base_color, theta, phi)
                        if actor is not None:
                            pl.renderer.AddActor(actor)
                else:
                    if n_atoms <= new_cfg.max_atoms_for_true_spheres:
                        baked = glyph_spheres(base_world, new_cfg.base_radius, new_cfg.sphere_theta, new_cfg.sphere_phi)
                        if baked is not None and baked.n_points:
                            pl.add_mesh(baked, color=new_cfg.base_color, smooth_shading=True, specular=0.2)
                    else:
                        add_points_impostor(pl, base_world, new_cfg.base_color, new_cfg.points_impostor_size)

                hydrogen_centers_world = np.empty((0, 3), dtype=float); hydrogen_radius = 0.0
                for mesh, sp in dop_meshes:
                    if mesh is not None and mesh.n_points:
                        pl.add_mesh(mesh, color=sp.color, smooth_shading=True, specular=0.25)
                for sp, centers_w, rad in dopant_world_centers:
                    if sp.mode == "interstitial" and sp.name.lower().startswith("h") and centers_w.size:
                        hydrogen_centers_world = centers_w; hydrogen_radius = rad; break

                if new_cfg.show_axes:
                    pl.add_axes()
                    if base_world is not None and base_world.size:
                        Lx = float(np.max(base_world[:,0]) - np.min(base_world[:,0]))
                        Ly = float(np.max(base_world[:,1]) - np.min(base_world[:,1]))
                        Lz = float(np.max(base_world[:,2]) - np.min(base_world[:,2]))
                        bounds = (0.0, round(Lx,6), 0.0, round(Ly,6), 0.0, round(Lz,6))
                        pl.show_bounds(bounds=bounds, show_xaxis=True, show_yaxis=True, show_zaxis=True,
                                       xtitle="x (nm)", ytitle="y (nm)", ztitle="z (nm)",
                                       location="outer", ticks="outside", fmt="%.2f", minor_ticks=False)

                if new_cfg.zoom_mode == "cursor":
                    enable_zoom_to_mouse(pl, sensitivity=1.0)
                else:
                    pl.enable_trackball_style()

                if new_cfg.show_unit_cell_overlay:
                    draw_unit_cell_overlay(pl, new_cfg)

                if new_cfg.enable_picking and not new_cfg.__dict__.get("no_show", False):
                    enable_picker(pl, new_cfg, hydrogen_centers_world, hydrogen_radius)

                _restore_camera(pl, cam_snap)
                pl.render()
            except Exception as _e:
                try:
                    show_toast(pl, f"Reload failed — {_e}", seconds=4.0, position="lower_right", bg="red", bg_opacity=0.85)
                except Exception:
                    pass

        
        # ---- Minimal control panel (sliders & toggles) ----
        # Helper to mutate cfg and rebuild the scene in place
        def _mutate_and_rebuild(mutator):
            nonlocal cfg
            try:
                mutator(cfg)
                _rebuild_scene_in_place(cfg)
                show_toast(pl, "Updated", seconds=0.8, position="lower_right", bg="yellow")
            except Exception as e:
                show_toast(pl, f"Update failed — {e}", seconds=3.0, position="lower_right", bg="red", bg_opacity=0.85)

        # Lattice type cycler
        _lats = ["SC", "BCC", "FCC"]
        def _cycle_lattice():
            idx = (_lats.index(cfg.lattice) + 1) % len(_lats) if cfg.lattice in _lats else 0
            _mutate_and_rebuild(lambda c: setattr(c, "lattice", _lats[idx]))
        try:
            pl.add_button_widget(lambda: _cycle_lattice(), position=(10, 10), size=32, style="rounded", color=None, font_size=10, border_size=1, text="Lattice")
        except Exception:
            pass

        # Render mode cycler
        _modes = ["auto", "spheres", "impostor_points"]
        def _cycle_mode():
            idx = (_modes.index(cfg.render_mode) + 1) % len(_modes) if cfg.render_mode in _modes else 0
            _mutate_and_rebuild(lambda c: setattr(c, "render_mode", _modes[idx]))
        try:
            pl.add_button_widget(lambda: _cycle_mode(), position=(10, 50), size=32, style="rounded", color=None, font_size=10, border_size=1, text="Render")
        except Exception:
            pass

        # Checkboxes
        try:
            pl.add_checkbox_button_widget(lambda state: _mutate_and_rebuild(lambda c: setattr(c, "show_axes", bool(state))),
                                          position=(10, 90), value=bool(cfg.show_axes))
            pl.add_text("Axes", position=(40, 90), font_size=10)
        except Exception:
            pass
        try:
            pl.add_checkbox_button_widget(lambda state: _mutate_and_rebuild(lambda c: setattr(c, "show_unit_cell_overlay", bool(state))),
                                          position=(10, 120), value=bool(cfg.show_unit_cell_overlay))
            pl.add_text("Unit cell", position=(40, 120), font_size=10)
        except Exception:
            pass
        try:
            pl.add_checkbox_button_widget(lambda state: _mutate_and_rebuild(lambda c: setattr(c, "enable_picking", bool(state))),
                                          position=(10, 150), value=bool(cfg.enable_picking))
            pl.add_text("Picking", position=(40, 150), font_size=10)
        except Exception:
            pass

        # Sliders (Nx, Ny, Nz) -- kept modest to avoid huge rebuilds
        try:
            pl.add_text("Nx", position=(10, 190), font_size=10)
            pl.add_slider_widget(lambda v: _mutate_and_rebuild(lambda c: setattr(c, "Nx", int(v))),
                                 rng=[1, 200], value=int(cfg.Nx), pointa=(0.05, 0.18), pointb=(0.35, 0.18), title=None, event_type="end")
        except Exception:
            pass
        try:
            pl.add_text("Ny", position=(10, 220), font_size=10)
            pl.add_slider_widget(lambda v: _mutate_and_rebuild(lambda c: setattr(c, "Ny", int(v))),
                                 rng=[1, 200], value=int(cfg.Ny), pointa=(0.05, 0.21), pointb=(0.35, 0.21), title=None, event_type="end")
        except Exception:
            pass
        try:
            pl.add_text("Nz", position=(10, 250), font_size=10)
            pl.add_slider_widget(lambda v: _mutate_and_rebuild(lambda c: setattr(c, "Nz", int(v))),
                                 rng=[1, 200], value=int(cfg.Nz), pointa=(0.05, 0.24), pointb=(0.35, 0.24), title=None, event_type="end")
        except Exception:
            pass

        # Sliders: base radius and impostor size
        try:
            pl.add_text("Base r", position=(10, 290), font_size=10)
            pl.add_slider_widget(lambda v: _mutate_and_rebuild(lambda c: setattr(c, "base_radius", float(v))),
                                 rng=[0.05, 2.0], value=float(cfg.base_radius), pointa=(0.05, 0.28), pointb=(0.35, 0.28), title=None, event_type="end")
        except Exception:
            pass
        try:
            pl.add_text("Impostor size", position=(10, 320), font_size=10)
            pl.add_slider_widget(lambda v: _mutate_and_rebuild(lambda c: setattr(c, "points_impostor_size", float(v))),
                                 rng=[0.5, 10.0], value=float(cfg.points_impostor_size), pointa=(0.05, 0.31), pointb=(0.35, 0.31), title=None, event_type="end")
        except Exception:
            pass

        # Helpful: quick button to reload from file (same as F5)
        try:
            pl.add_button_widget(lambda: _reload_now(), position=(10, 360), size=32, style="rounded", color=None, font_size=10, border_size=1, text="Reload")
        except Exception:
            pass

    def _reload_now():
        import time
        _state["last_reload_time"] = time.time() * 1000.0
        try:
            new_cfg = load_config(cfg_path)
            normalize_physical_config(new_cfg)
            assign_random_positions(new_cfg, new_cfg.dopants)
        except Exception as e:
            show_toast(pl, f"Reload: config error — {e}", seconds=4.0, position="lower_right", bg="red", bg_opacity=0.85)
            return
        nonlocal cfg, _last_fp
        _rebuild_scene_in_place(new_cfg)
        cfg = new_cfg
        _last_fp = _file_fingerprint(cfg_path)
        show_toast(pl, "Config reloaded ✔", seconds=1.4, position="lower_right", bg="yellow")

    def _try_auto_reload(_=None):
        import time
        now_ms = time.time() * 1000.0
        fp = _file_fingerprint(cfg_path)
        if fp and fp != _last_fp and (now_ms - _state["last_reload_time"]) > _reload_cooldown_ms:
            _reload_now()

    # Bind F5
    try:
        pl.iren.add_observer("KeyPressEvent",
            lambda obj, ev: _reload_now() if obj.GetKeySym().upper() == "F5" else None)
    except Exception:
        try:
            pl.add_key_event("f5", _reload_now)
        except Exception:
            pass

    # Poll every 500 ms
    try:
        pl.add_callback(_try_auto_reload, interval=500)
    except Exception:
        pass
...
Entry point wiring: config, normalization, placements, optional dump, run plot.
    ...
    plot(cfg, export_dir=args.export_dir, export_merged=args.export_merged, screenshot=args.screenshot, no_show=args.no_show, cfg_path=config_path)
